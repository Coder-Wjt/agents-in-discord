...
- Updated the multi-platform plan and deployment checklist for native Lark cards/forms, slash commands, group reply-chain child conversations, verified webhook deployment, completed real-credential smoke coverage, and rollback operations; Slack remains a later phase.
- Marked the Lark milestone complete after production acceptance, added dated capability and verification checkpoints, and prioritized the remaining roadmap as unified observability, Discord conversation-key migration tooling, then Slack.
...
- Completed the single-runtime production webhook acceptance with all receipt evidence present: signed and encrypted requests, URL challenge, ordinary message, native slash command, bot-menu event, card action, application restart recovery, and independent Cloudflare interruption/recovery. The old CLI runtime is disabled, the webhook and tunnel services remain enabled, and the published Open Platform configuration was manually confirmed where the read-only API could not verify it.
...
- Verified the temporary public webhook edge through real HTTPS: the health probe, encrypted URL challenge, signed/encrypted dispatcher event, generic invalid-signature rejection, and same-tunnel listener restart recovery all passed. The later single-runtime production acceptance separately completed the real Open Platform coverage.

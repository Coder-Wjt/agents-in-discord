...
| 恢复与指标 | 待验收 | 自动化覆盖重连、自愈、连接/投递快照和优雅退出 | 完成断网恢复、`!status` 指标核对和 SIGTERM smoke |
